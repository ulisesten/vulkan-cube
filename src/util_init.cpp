...
VkResult init_enumerate_device(struct Info &info, uint32_t gpu_count) {
...
#ifdef _WIN32
static void run(struct Info *info) { /* Placeholder for samples that want to show dynamic content */ }
...
void init_swap_chain(struct Info &info, VkImageUsageFlags usageFlags) {
...
void init_depth_buffer(struct Info &info) {
    ...
    if (props.linearTilingFeatures & VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT) {
        ...
        printf("%s depth_format Unsupported.\n",__func__);
        exit(-1);
    }
...
void init_descriptor_and_pipeline_layouts(struct Info &info, bool use_texture) {
...
void init_renderpass(struct Info &info, bool include_depth, bool clear, VkImageLayout finalLayout) {
...
void init_shaders(struct Info &info, const char *vertShaderText, const char *fragShaderText) {
    VkResult U_ASSERT_ONLY res;
...
void init_vertex_buffer(struct Info &info, const void *vertexData, uint32_t dataSize, uint32_t dataStride,
                        bool use_texture) {
    VkResult U_ASSERT_ONLY res;
    bool U_ASSERT_ONLY pass;
...
void init_pipeline(struct Info &info, VkBool32 include_depth, VkBool32 include_vi) {
    VkResult U_ASSERT_ONLY res;
...
/**Destructors*/
void destroy_pipeline(struct Info &info) { vkDestroyPipeline(info.device, info.pipeline, NULL); }
void destroy_pipeline_cache(struct Info &info) { vkDestroyPipelineCache(info.device, info.pipelineCache, NULL); }
void destroy_descriptor_pool(struct Info &info) { vkDestroyDescriptorPool(info.device, info.desc_pool, NULL); }
...
void destroy_renderpass(struct Info &info) { vkDestroyRenderPass(info.device, info.render_pass, NULL); }
...
void destroy_command_pool(struct Info &info) { vkDestroyCommandPool(info.device, info.cmd_pool, NULL); }
...
void destroy_instance(struct Info &info) { vkDestroyInstance(info.inst, NULL); }
